...
class MyScanCheck implements ScanCheck {
    ...
    List<String> urlWhitelistList = new ArrayList<>();
    List<String> paramWhitelistList = new ArrayList<>();;
    ...
    MyScanCheck(MontoyaApi api) {
        ...
        CustomPanel.getConfirmButton().addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                String urlWhitelistText = CustomPanel.getUrlWhitelistArea().getText();
                urlWhitelistList = Arrays.asList(urlWhitelistText.split("\\s*,\\s*"));
                String paramWhitelistText = CustomPanel.getParamWhitelistArea().getText();
                paramWhitelistList = Arrays.asList(paramWhitelistText.split("\\s*,\\s*"));
    ...
    public HttpRequestResponse sendrequest(HttpRequest request){
        try {
            Thread.sleep(Integer.parseInt(sleep_time)*1000);
    ...
    public AuditResult passiveAudit(HttpRequestResponse baseRequestResponse) {
        ...
        try {
            ...
            if(urlWhitelistList.contains(request_url.getHost())){
                return auditResult();
            }
        ...
        for (ParsedHttpParameter parameter : parameters) {
            ...
            if(paramWhitelistList.contains(parameter.name())){
                continue;
            }
