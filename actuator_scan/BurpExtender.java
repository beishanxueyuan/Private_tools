...
public class BurpExtender implements IBurpExtender, IScannerCheck {
    ...
    private static final String EXTENSION_NAME = "actuator scan";
    ...
    public List<IScanIssue> doPassiveScan(IHttpRequestResponse baseRequestResponse) {
        ...
        executorService.execute(()-> {
            ...
            for (int i = 0; i < pathSegments.length; i++) {
                full_path = full_path  +"/"+ pathSegments[i];
                baseUrl = baseUrl + "/" + pathSegments[i];
                if(pathSegments[i].matches("\\d+")){
                    continue;
                }
                String actuator_path= (full_path+"/actuator").replace("//","/");
                ...
                newRequestString = newRequestString.replaceFirst(path, actuator_path).replaceFirst(requestInfo.getMethod(),"GET");
